...
#### Поля класса:
`readonly baseUrl: string` - базовый url для запроса на сервер\
`protected options: RequestInit` - опции для настройки запроса на сервер\
...
## класс EventEmitter
Реализует паттерн «Наблюдатель», позволяет подписываться на события и уведомлять подписчиков о наступлении события.

### Методы класса
`on` - установлевает обработчик на событие\
`off` - снимает обработчик с события\
`emit` - инициирует событие с данными\
`onAll` - слушает все события\
`offAll` - сбрасывает все обработчики\
`trigger` - генерирует заданное событие при вызове с заданными аргументами\

## Слой модели данных

### Абстрактный класс Model<T>

Абстрактный базовый класс, предназначенный для создания моделей данных.
...
`orderContacts:open` -  открыть форму с контактами\
`card:select` - выбор карточки для отображения в модальном окне\
`preview:changed` - открытие окна карточки\
`basket:update` - изменение корзины\
